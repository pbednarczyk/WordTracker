...
final class PublicationVocabularyRepository extends ServiceEntityRepository
{
    ...
    public function getCoverageStats(Publication $publication): array
    {
        $row = $this->createQueryBuilder('pv')
            ...
            ->addSelect('COALESCE(SUM(CASE WHEN vi.status IN (:knownStatuses) THEN 1 ELSE 0 END), 0) AS uniqueKnown')
            ->addSelect('COALESCE(SUM(CASE WHEN vi.status NOT IN (:knownStatuses) THEN 1 ELSE 0 END), 0) AS uniqueUnknown')
            ->addSelect('COALESCE(SUM(pv.occurrences), 0) AS occurrencesTotal')
            ->addSelect('COALESCE(SUM(CASE WHEN vi.status IN (:knownStatuses) THEN pv.occurrences ELSE 0 END), 0) AS occurrencesKnown')
            ->addSelect('COALESCE(SUM(CASE WHEN vi.status NOT IN (:knownStatuses) THEN pv.occurrences ELSE 0 END), 0) AS occurrencesUnknown')
            ->andWhere('pv.publication = :publication')
            ->andWhere('pv.deletedAt IS NULL')
            ->setParameter('publication', $publication)
            ->setParameter('knownStatuses', VocabularyStatus::knownForCoverage())
            ->getQuery()
            ->getSingleResult();

...
final readonly class LearningCardGenerator
{
    ...
    /** Refresh existing active generated cards; the caller owns the transaction and flush. */
    public function synchronize(PublicationVocabulary $publicationVocabulary): void
    {
        $enrichment = $publicationVocabulary->getEnrichment();
        if ($publicationVocabulary->isDeleted() || $enrichment === null) {
            return;
        }

        $candidates = [];
        foreach ($this->buildCandidates($publicationVocabulary, $enrichment->getSourceSentence()) as $candidate) {
            // CLOZE content is not generated by the current policy.
            if ($candidate['type'] !== LearningCardType::CLOZE) {
                $candidates[$candidate['type']->value] = $candidate;
            }
        }

        foreach ($this->learningCardRepository->findBy([
            'publicationVocabulary' => $publicationVocabulary,
            'isActive' => true,
        ]) as $card) {
            $candidate = $candidates[$card->getType()->value] ?? null;
            if ($candidate === null || $card->getPublicationVocabularyEnrichment() === null) {
                continue;
            }

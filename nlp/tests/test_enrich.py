...
from wordtracker_nlp.main import analyzer, app, get_llm_client
...
from wordtracker_nlp.ollama import OllamaClient, OllamaConfig
...
class FakeLlmClient:
    def __init__(
        ...
        self.result = result or Enrichment(
    ...
    def generate(self, prompt: str, format_schema: dict[str, Any], options: dict[str, Any]) -> str:
        call = {"prompt": prompt, "format_schema": format_schema, "options": options}
        if format_schema == REPAIRED_SIMPLE_EXAMPLE_FORMAT_SCHEMA:
            self.repair_requests.append(call)
            example = self.repair_results.pop(0) if self.repair_results else self.result.simple_example
            return json.dumps({"simple_example": example})
...
def test_enrich_uses_generic_llm_dependency_and_returns_metadata() -> None:
    fake = FakeLlmClient()
    ...
    assert '"lemma": "willingness"' in fake.requests[0]["prompt"]
    assert fake.requests[0]["format_schema"] == ENRICHMENT_FORMAT_SCHEMA
    assert fake.requests[0]["options"] == {"temperature": 0.2}
...
def test_ollama_client_sends_model_prompt_and_structured_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = generate_enrichment(OllamaClient(
...
def test_enrich_repairs_simple_example_without_regenerating_other_fields() -> None:
    ...
    repair_call = fake.repair_requests[0]
    assert repair_call["format_schema"] == REPAIRED_SIMPLE_EXAMPLE_FORMAT_SCHEMA
    assert repair_call["options"] == {"temperature": 0.2}
    repair_data = json.loads(repair_call["prompt"].split("REPAIR DATA JSON:\n")[1])
    assert repair_data["validation_error"] == {
...
def test_enrich_skips_repair_when_first_attempt_is_valid() -> None:
    fake = FakeLlmClient(result=alter_enrichment("We need to alter the plan."))
    app.dependency_overrides[get_llm_client] = lambda: fake
...
def test_enrich_falls_back_to_context_sentence_when_repair_is_invalid() -> None:
    ...
    assert len(fake.requests) == 1
    assert len(fake.repair_requests) == 1
    repair_data = json.loads(fake.repair_requests[0]["prompt"].split("REPAIR DATA JSON:\n")[1])
    assert repair_data["rejected_simple_example"] == "The accident changed his life."
...
def test_transport_failure_does_not_attempt_semantic_repair() -> None:
    fake = FakeLlmClient(error=TimeoutError("Ollama timed out."))
    app.dependency_overrides[get_llm_client] = lambda: fake
...
def test_ollama_client_uses_only_configured_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    generate_enrichment(OllamaClient(OllamaConfig(model="qwen3:14b")), EnrichRequest.model_validate(enrich_request()))
...
def test_enrichment_rejects_invalid_model_json(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(httpx, "post", lambda *args, **kwargs: httpx.Response(200, json={"response": "not json"}))

    with pytest.raises(ValueError, match="invalid JSON"):
        generate_enrichment(OllamaClient(), EnrichRequest.model_validate(enrich_request()))


def test_enrichment_rejects_invalid_cefr(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        httpx,
        "post",
...
def test_ollama_client_handles_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(TimeoutError, match="timed out"):
        generate_enrichment(OllamaClient(OllamaConfig(timeout_seconds=60)),
            EnrichRequest.model_validate(enrich_request())
        )
...
def test_enrich_maps_timeout_to_504() -> None:
    app.dependency_overrides[get_llm_client] = lambda: FakeLlmClient(error=TimeoutError("Ollama timed out."))
...
def test_enrich_maps_connection_error_to_503() -> None:
    app.dependency_overrides[get_llm_client] = lambda: FakeLlmClient(error=ConnectionError("Ollama unavailable."))
...
def alter_enrichment(simple_example: str) -> Enrichment:
    return Enrichment(
...
@pytest.mark.parametrize("raw", ["", "not json", '{}', '{"cefr_level":"B3"}'])
def test_enrich_parses_raw_output_from_generic_client(raw: str) -> None:
    class RawClient:
        provider = "test"
        model = "test-model"

        def generate(self, prompt, format_schema, options):
            return raw

    app.dependency_overrides[get_llm_client] = RawClient
    response = TestClient(app).post("/enrich", json=enrich_request())
...
@pytest.mark.parametrize("error, status", [
    ...
    (RuntimeError("upstream error"), 502),
    (ValueError("bad envelope"), 502),
])
def test_enrich_maps_generic_repair_failures(error: Exception, status: int) -> None:
    class RepairFailureClient(FakeLlmClient):
        def generate(self, prompt, format_schema, options):
            if format_schema == REPAIRED_SIMPLE_EXAMPLE_FORMAT_SCHEMA:
                raise error
            return super().generate(prompt, format_schema, options)

    fake = RepairFailureClient(result=alter_enrichment("The weather is nice."))
    app.dependency_overrides[get_llm_client] = lambda: fake
    response = TestClient(app).post("/enrich", json=alter_request().model_dump())
...
def test_enrich_rejects_invalid_context_fallback() -> None:
    fake = FakeLlmClient(
        result=alter_enrichment("The weather is nice."),
        repair_results=["The weather is warm."],
    )
    app.dependency_overrides[get_llm_client] = lambda: fake
    payload = alter_request().model_dump()
    payload["context_sentence"] = "The weather is sunny."
    response = TestClient(app).post("/enrich", json=payload)
    assert response.status_code == 502
    assert response.json()["detail"] == (
        "Enrichment validation failed after 1 repair attempts: "
        "field=simple_example code=TARGET_NOT_PRESENT"
    )
    assert len(fake.repair_requests) == 1


def test_ollama_transport_accepts_generic_schema_and_options(monkeypatch: pytest.MonkeyPatch) -> None:
    captured = {}

    def fake_post(url, json, timeout):
        captured.update(json)
        return httpx.Response(200, json={"response": "raw model output"})

    monkeypatch.setattr(httpx, "post", fake_post)
    client = OllamaClient(OllamaConfig(model="generic-model"))
    schema = {"type": "object", "properties": {"summary": {"type": "string"}}}
    options = {"temperature": 0.7, "num_predict": 42}
    assert client.generate("Summarize this text", schema, options) == "raw model output"
    assert captured == {
        "model": "generic-model", "prompt": "Summarize this text",
        "format": schema, "options": options, "stream": False,
    }
    assert client.provider == "ollama"
    assert client.model == "generic-model"
